...
const InitialOverlay: React.FC<InitialOverlayProps> = ({ onStartQuiz, onGoToSite }) => {
  return (
    <div className="fixed inset-0 z-[100] flex flex-col items-center justify-center p-6 bg-black overflow-hidden">
      {/* Background Image with Overlay - Same as Quiz for continuity */}
      <div className="absolute inset-0 z-0">
        <img 
          src={IMAGES.hero} 
          alt="Background" 
          className="w-full h-full object-cover object-top opacity-60 scale-110 blur-[2px]"
        />
        <div className="absolute inset-0 bg-gradient-to-b from-black/80 via-black/40 to-black/95 backdrop-blur-sm"></div>
      </div>
      ...
      <div className="max-w-md w-full space-y-8 text-center relative z-10">
        <div className="relative w-32 h-32 mx-auto mb-6">
          ...
          <img 
            ...
            className="w-full h-full object-cover rounded-full border-4 border-gold-500 shadow-[0_0_40px_rgba(212,175,55,0.4)]"
          />
        </div>
        
        <div className="space-y-3">
          <h1 className="text-3xl font-playfair font-bold text-white leading-tight drop-shadow-lg">Bem-vinda ao Universo <br /><span className="text-gold-400">Dra. Isadora Luísa</span></h1>
          <p className="text-gray-300 leading-relaxed text-sm font-medium drop-shadow-md">
        ...
        <div className="space-y-4 pt-4">
          <button 
            onClick={onStartQuiz}
            className="w-full py-5 px-8 gold-gradient text-black rounded-2xl font-black shadow-2xl shadow-gold-900/40 transition-all transform active:scale-95 flex flex-col items-center btn-pulse"
          >
